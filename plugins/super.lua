...
local function check_member_super(cb_extra, success, result) 
  ...
  if success == 0 then 
   send_large_msg(receiver, "☻لتلعب بكيفك للمدراء والادمنيه فقط ♯") 
  end 
  for k,v in pairs(result) do 
    ...
    if member_id ~= our_id then 
      ...
      save_data(_config.moderation.data, data) 
     local text = '♛♯تم تفعيل البوت في المجموعه 🎈' 
  ...
  for k,v in pairs(result) do 
    ...
    if member_id ~= our_id then 
     -- Group configuration removal 
      ...
      save_data(_config.moderation.data, data) 
     local text = '☻تم تعطيل البوت في المجموعه ☠ℹ️' 
      return reply_msg(msg.id, text, ok_cb, false) 
    end 
  end 
end 
...
local title ="اعدادات المجموعه 🌞🎈: ["..result.title.."]\n\n" 
...
local function lock_group_links(msg, data, target) 
  ...
  if group_link_lock == 'yes' then 
    return 'ℹالروابط بالتاكيد تم✔ قفلها مسبقا ☻' 
  else 
...
local function unlock_group_links(msg, data, target) 
  ...
  if group_link_lock == 'no' then 
    return '♛بالتاكيد تم ✔فتح الروابط ♯' 
  else 
    data[tostring(target)]['settings']['lock_link'] = 'no' 
    save_data(_config.moderation.data, data) 
    return '♯تم✔ فتح الروابط في المجموعه يمكنكم ارسال الروابط☻' 
  end 
end 
...
local function lock_group_spam(msg, data, target) 
  ...
  if not is_owner(msg) then 
    return "♯ليست لديك صلاحيه ☹ لتحكم في البوت ☠ لضل زعطوط ☻" 
  end 
  local group_spam_lock = data[tostring(target)]['settings']['lock_spam'] 
  if group_spam_lock == 'yes' then 
    return '✔بالتاكيد تم قفل مضاد السبام في المجموعه♠' 
  else 
    data[tostring(target)]['settings']['lock_spam'] = 'yes' 
    save_data(_config.moderation.data, data) 
    return '♯تم✔ قفل 🔐 مضاد السبام بنجاح ☹' 
  end 
end 
...
local function unlock_group_spam(msg, data, target) 
  ...
  if group_spam_lock == 'no' then 
    return 'بالتاكيد تم ✔ فتح مضاد السبام ☄ في المجموعه ☹' 
  else 
    data[tostring(target)]['settings']['lock_spam'] = 'no' 
    save_data(_config.moderation.data, data) 
    return 'تم ✔فتح مضاد الاسبام في المجموعه ☹️' 
  end 
end 
...
local function lock_group_flood(msg, data, target) 
  ...
  if group_flood_lock == 'yes' then 
    return '☻التكرار بالفعل مقفول🔐 عزيزي♯' 
  else 
    data[tostring(target)]['settings']['flood'] = 'yes' 
    save_data(_config.moderation.data, data) 
    return 'تم✔ قفل 🔐 التكرار ☄ في المجموعه️' 
  end 
end 
...
local function promote_admin(receiver, member_username, user_id) 
  ...
  if data[group]['moderators'][tostring(user_id)] then 
    return send_large_msg(receiver, member_username..'بالتاكيد تم ✔ رفعه ادمن في المجموعه ☻') 
  end 
...
local function promote2(receiver, member_username, user_id) 
  ...
  if data[group]['moderators'][tostring(user_id)] then 
    return send_large_msg(receiver, member_username..'بالتاكيد تم ✔ رفع ادمن  في المجموعه ☻') 
  end 
  data[group]['moderators'][tostring(user_id)] = member_tag_username 
  save_data(_config.moderation.data, data) 
...
function get_message_callback(extra, success, result) 
   ...
   local name_log = print_name:gsub("_", " ") 
    if get_cmd == "19-*" and not result.action then 
   ...
   elseif get_cmd == 'ايدي' and result.action then 
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then 
            return send_large_msg("channel#id"..channel_id, "لا يمكنك ☠ طرد الادمنيه ☻ والمدراء ☹ ") 
    end 
   ...
   elseif get_cmd == 'channel_block' and result.action and result.action.type == 'chat_add_user' then 
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then 
            return send_large_msg("channel#id"..channel_id, "لا يمكنك ☠ طرد الاداريين ☄ والمدراء ☻ ") 
    end 
    if is_admin2(member_id) then 
   ...
   elseif get_cmd == "رفع اداري" then 
      ...
      if result.from.username then 
         text = "@"..result.from.username.."تم ✔ ترقيتك ☻ اداري  في المجموعه 🌝" 
      else 
   ...
   elseif get_cmd == "تنزيل اداري" then 
      ...
      if is_admin2(result.from.peer_id) then 
         return send_large_msg(channel_id, "ليست ☠ لديك ✘ صلاحيه تنزيل اداراي ☻") 
      end 
...
local function cb_user_info(extra, success, result) 
   ...
   if get_cmd == "تنزيل اداري" then 
      if is_admin2(result.peer_id) then 
         return send_large_msg(receiver, "ليست ☠ لديك صلاحيه تنزيل الادمينه والمدراء ☻") 
      end 
      local user_id = "user#id"..result.peer_id 
...
local function callbackres(extra, success, result) 
  ...
  local get_cmd = extra.get_cmd 
   if get_cmd == "19788" then 
      local user = result.peer_id 
      local name = string.gsub(result.print_name, "_", " ") 
      local channel = 'channel#id'..extra.channelid 
      send_large_msg(channel, user..'\n'..name) 
      return user 
   elseif get_cmd == "19*-" then 
      local user = result.peer_id 
      local channel = 'channel#id'..extra.channelid 
      send_large_msg(channel, user) 
      return user 
    ...
    if vusername == member or vpeer_id == memberid then 
     ...
     local sender = cb_extra.msg.from.id 
      ...
      if is_momod2(user_id, channel_id) and not is_admin2(sender) then 
        return send_large_msg("channel#id"..channel_id, "ليست ☠ لديك صلاحيه ☹ طرد الادمنيه والمدراء ☻") 
      end 
   ...
   local data = load_data(_config.moderation.data) 
      ...
      if matches[1] == 'الرابط' then 
         ...
         if not group_link then 
            return "يرجى ارسال ❗♯ ️تغير الرابط ♯ ☻ لانشاء 🌝 رابط المجموعه ✍" 
         end 
      ...
      if matches[1] == '2*77' and is_owner(msg) then 
         local cbres_extra = { 
            channelid = msg.to.id, 
            get_cmd = '367' 
         } 
      ...
      if matches[1] == 'قفل' and is_momod(msg) then 
         ...
         local msg_type = 'Documents' 
            if not is_muted(chat_id, msg_type..': yes') then 
               savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type) 
               mute(chat_id, msg_type) 
               return msg_type.."تم ☑️ قفل 🔒 الملفات ✋😽" 
            else 
               return "SuperGroup mute "..msg_type.."الملفات بالفعل ✔️ تم قفلها 🔒" 
            end 
         end 
      ...
      if matches[1] == 'فتح' and is_momod(msg) then 
         ...
         local msg_type = 'Documents' 
            if is_muted(chat_id, msg_type..': yes') then 
               savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type) 
               unmute(chat_id, msg_type) 
               return msg_type.."تم ✔️ فتح 🔓 الملفات" 
            else 
               return "Mute "..msg_type.."الملفات بالفعل ✔️ تم فتحها 🔓☹" 
            end 
         end 
         if matches[2] == 'الدردشه' then 
         local msg_type = 'Text' 
